...
/**
 * @note:  These policy bitmaps are in big endian format.
 *         see NXP app note AN12413, Rev 2.12 March 24, 2021 section 4.3.34.
 */
...
typedef struct
{
    ...
    bool has_session;
    uint32_t auth_id;   /* key stored in se050 used to authenticate */
...
typedef struct
{
    uint8_t len;           ///< policy byte len
    uint32_t object_id;    ///< Secure object ID policies are applied to.
    uint32_t policy_bits;  ///< Policy bitmap
} __attribute__((packed)) Se05x_policy_t;


typedef enum
{
    kSE05x_AuthType_None = 0,

    /** Global platform SCP03 */
    kSE05x_AuthType_SCP03 = 1,

    /** (e.g. SE05X) UserID based connection */
    kSE05x_AuthType_ID = 2,

    /** (e.g. SE05X) Use AESKey for user authentication
     *
     *  Earlier this was called  kSSS_AuthType_AppletSCP03
     */
    kSE05xAuthType_AESKey = 3,

    /** (e.g. SE05X) Use ECKey for user authentication
     *
     *  Earlier this was called  kSSS_AuthType_FastSCP
     */
    kSE05x_AuthType_ECKey = 4,
     ...
     */
    kSE05x_AuthType_INT_ECKey_Counter = 0x14,
...
typedef enum
{
    ...
    kSE050_ECCurve_MAX_id    = 0x43   ///< Max curve id
} SE05x_ECCurve_t;
